...
const actions = {
  ...
  async loadFilteredArticlesByTag({ commit, state }, currentCategory) {
    try {
      // 需要多带一个标签参数
      const response = await axios.get(`http://www.dyp02.vip:3000/articles/ByTag/PageCtrl?page=${state.currentPageByTag}&size=${state.pageSizeByTag}&currentCategory=${currentCategory}`);
