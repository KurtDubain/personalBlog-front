...
const actions = {
  ...
  async loginUserFromArticles({ dispatch, commit }, { formData, remBtn }) {
    try {
      // 发送登陆以及评论表单
      await axios.post(
        `https://www.dyp02.vip:443/backend/comments/form`,
        formData
      );
      // 获取用户数据
      const resUser = await axios.get(
        `https://www.dyp02.vip:443/backend/users/GetTokenById/${formData.username}`
      );
  ...
  async loginUserFromChats({ dispatch, commit }, { formData, remBtn }) {
    try {
      // 发送留言表单
      await axios.post(
        "https://www.dyp02.vip:443/backend/chats/postChatComment",
        formData
      );
      const resUser = await axios.get(
        `https://www.dyp02.vip:443/backend/users/GetTokenById/${formData.username}`
  ...
  async loginUserWithoutForm({ dispatch, commit }, { formData, remBtn }) {
    try {
      await axios.post(
        "https://www.dyp02.vip:443/backend/users/FromChatLogin",
        formData
      );
      const resUser = await axios.get(
        `https://www.dyp02.vip:443/backend/users/GetTokenById/${formData.username}`
  ...
  checkRememberedLogin({ dispatch, commit }) {
    ...
    if (
      rememberedLoginTime &&
      currentTime - rememberedLoginTime > sevenDaysInMillis
    ) {
      // 如果超过7天，执行清除操作
      localStorage.removeItem("rememberedLogin");
      localStorage.removeItem("userID");
  ...
  async getUserInfo({ commit }, userID) {
    try {
      const res = await axios.get(
        `https://www.dyp02.vip:443/backend/users/getUserInfo/${userID}`
      );
